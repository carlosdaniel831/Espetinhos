...
function finalizarPedido(){
  ...
  const pagamento = document.getElementById('pagamento').value; // pode ser vazio
  ...
  db.ref('pedidos').push({
    ...
    itens,
    total,
    pagamento, // string vazia = não informado
    pago:      false,
    entregue:  false,
...
function abrirModal(key){
  ...
  // Montar carrinho de edição a partir dos itens do pedido
  editCarrinho = {};
  Object.keys(produtos).forEach(id => { editCarrinho[id] = 0; });

  // Mapear nomes dos itens de volta para os ids dos produtos
