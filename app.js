...
function salvarConfigFirebase(){
  ...
  const url = document
    .getElementById('inputFirebaseUrl')
    .value
    .trim();
...
function iniciarFirebase(url){
  ...
  try{
    ...
    toast('Erro ao conectar Firebase', true);
  }
}
...
function calcularNumero(){
  ...
  const nums = Object.values(pedidos)
    .map(p => p.id || 0);
...
function finalizarPedido(){
  ...
  if(!Object.values(carrinho).some(q => q > 0)){
    toast('Adicione itens ao pedido', true);
    return;
  }
  ...
  .catch(() => {
    ...
    toast('Erro ao salvar pedido', true);
...
function abrirModal(key){
  ...
  for(const [nomeCompleto, qtd] of Object.entries(p.itens || {})){
    ...
    for(const [id, prod] of Object.entries(produtos)){
      ...
      if(nomeCompleto.startsWith(prod.nome)){
